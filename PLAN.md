...
| **PR3** | Add favorites endpoint, auth middleware, and favorite button | `packages/types` (adds `ownerId` to `Pokemon`), `apps/api/src/middleware/auth.ts`, `apps/api/src/routes/favorites.ts`, `apps/web/components/FavoriteButton.tsx` | **Possible semantic conflict with PR2** — PR3 changes `Pokemon` type to include required `ownerId`. *If* PR2's `app/page.tsx` constructs Pokemon objects without it (depends on what the agent generated), the type drift surfaces as a typecheck failure once PR2 merges and PR3 rebases. One-line fix: `ownerId?: string`. Dry-run 1 showed the agent often pre-empts this by propagating `ownerId` through `page.tsx` while building the stack, in which case PR3 lands clean — see §7 Act 3 step 3 for both narrations. |
...
| `test_api` | `pnpm --filter api test` | Where the flaky test lives. |
...
**A note on batching.** The queue rule sets `batch_size: 5`, so Mergify will combine queued PRs that share a head-of-queue moment into a single batched CI run. With a 3-PR stack, the planted PR2 failure breaks this into two batches: PR1 alone (PR2 is gated by failing lint), and then PR2+PR3 together after the fix-and-restack. The beat numbering below stays the same, but step 3 (PR3 lands) often happens *inside the same CI run* as step 2's re-queue — sometimes you'll see PR2 and PR3 land seconds apart instead of in two separate cycles. Either is fine; narrate what you actually see.
...
3. **PR3 rebases and lands (~3 min).** Mergify auto-rebases PR3 onto the new `main` (which has PR1 and PR2's slices) and re-runs CI. Two paths from here — both fine to narrate:

   **(a) Clean rebase, no conflict.** PR3's `ownerId: string` is required in `packages/types`, but the agent already propagated it through every file it touched on the way up the stack, so the rebased state typechecks cleanly. Narrate: *"This is the upside of a thoughtfully constructed stack — Claude saw the type change and threaded it through `page.tsx` on the way up, so nothing breaks at the rebase. Stack design pays off."* CI green. PR3 squash-merges.

   **(b) Typecheck fails on the rebase.** `ownerId` was added as a required field but PR2's `apps/web/app/page.tsx` (now on `main`) constructs Pokemon objects without it. **Show the diff. This is the conflict the audience cares about: git couldn't see it, but the stack rebase did.** One-line fix: make `ownerId?: string` optional. `git commit --amend`, `mergify stack push`, `@mergifyio queue` on PR3 one more time. CI green. Merges.

   Dry-run 1 produced path (a) — Claude's pre-emptive consistency suppressed the conflict. Dry-run 2 produced path (b) — typecheck failed in `apps/api/test/sort.spec.ts` because PR1's test fixtures construct `Pokemon` literals without `ownerId`. The runbook is written to work either way; rehearse both narrations.

   Run `mergify stack list` in the terminal — empty. The stack is gone because it's all on `main`.

   **Flake re-roll moment:** If PR1's CI rolled into the safe bucket on its first pass and you didn't get the `max_checks_retries` beat at the head of the queue, you may still see the flake fire here as PR3's rebase re-runs the api tests against `main`. If you want to demonstrate the retry behaviour at this point, run `gh run rerun --failed <run-id>` — it usually re-rolls clean.
